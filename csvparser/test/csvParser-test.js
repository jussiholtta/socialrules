...
describe("csvp.Parser", function() {
    ...
    it("is able to create a card out of a line", function() {
        var arr = parser.readTextArea(myFile);
        var s = parser.parseLine(arr);
        var card = parser.createCard(s);
        expect(card.text).toEqual("Try to do the exact opposite of all the rules");
        expect(card.level).toEqual(3);
        expect(card.gm1).toBe(true);
        expect(card.gm2).toBe(true);
        expect(card.gm3).toBe(true);
        expect(card.amount).toEqual(1);
    });
